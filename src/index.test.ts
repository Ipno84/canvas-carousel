...
const baseOptions: CanvasCarouselOptions = { aspectRatio: 0.6, resizeTimeoutReaction: 1 }
const images: string[] = [
...
describe('Canvas carousel module', () => {
	...
	test('Check selector', () => {
		const canvasCarousel = new CanvasCarousel(selector, images, baseOptions);
		expect(canvasCarousel.selector).toBe(selector);
	});

	test('Check images', () => {
		const canvasCarousel = new CanvasCarousel(selector, images, baseOptions);
		expect(canvasCarousel.imagesPath).toMatchObject(images);
	});

	test('Check options', () => {
		const canvasCarousel = new CanvasCarousel(selector, images, baseOptions);
		expect(canvasCarousel.options).toMatchObject({ aspectRatio: 0.6, resizeTimeoutReaction: 1 });
	});
	...
	test('Check onInit called', () => {
		...
		new CanvasCarousel(selector, images, baseOptions);
	...
	test('Check canvas dimensions - with static dimensions options', () => {
		const canvasCarousel = new CanvasCarousel(selector, images, { ...baseOptions, aspectRatio: void 0, canvasWidth: 600, canvasHeight: 400 });
	...
	test('Check images collection', () => {
		...
		const mockImagesCollection = jest.spyOn(canvasCarousel, 'imagesCollection', 'get').mockImplementation(() => images.map((src) => {
	...
	test('Check resize observer - with undefined aspectRatio', () => {
		const canvasCarousel = new CanvasCarousel(selector, images, { ...baseOptions, aspectRatio: void 0 });
